...
export async function createHeroSlide(payload: Omit<HeroSlide, 'id'>): Promise<void> {
    const supabase = await createClient()
    const { error } = await (supabase as any).from('hero_slides').insert(payload)
...
export async function updateHeroSlide(id: string, payload: Partial<Omit<HeroSlide, 'id'>>): Promise<void> {
    const supabase = await createClient()
    const { error } = await (supabase as any).from('hero_slides').update(payload).eq('id', id)
    if (error) throw new Error(error.message)
    revalidatePath('/')
    revalidatePath('/admin/hero')
}
...
export async function reorderHeroSlides(orderedIds: string[]): Promise<void> {
    ...
    await Promise.all(
          orderedIds.map((id, index) =>
                  (supabase as any).from('hero_slides').update({ position: index }).eq('id', id)
                             )
        )
    revalidatePath('/')
    revalidatePath('/admin/hero')
