...
const CATEGORIES = ['Grundlagen', 'Wissen', 'Erfahrungen', 'Team']

interface Props {
  initial?: Partial<BlogPostInput> & { id?: string }
  onSave: (input: BlogPostInput) => Promise<{ error?: string }>
}
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-3xl space-y-5">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          <label className="mb-1 block text-xs font-bold uppercase tracking-widest text-muted-foreground">Titel</label>
        ...
        <div>
          <label className="mb-1 block text-xs font-bold uppercase tracking-widest text-muted-foreground">Slug (URL)</label>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          <label className="mb-1 block text-xs font-bold uppercase tracking-widest text-muted-foreground">Kategorie</label>
        ...
        <div>
          <label className="mb-1 block text-xs font-bold uppercase tracking-widest text-muted-foreground">Lesezeit (Min.)</label>
      ...
      <div>
        <label className="mb-1 block text-xs font-bold uppercase tracking-widest text-muted-foreground">Kurztext (max. 200 Zeichen)</label>
      ...
      <div>
        <label className="mb-1 block text-xs font-bold uppercase tracking-widest text-muted-foreground">Inhalt (Markdown)</label>
      ...
      <div>
        <label className="mb-1 block text-xs font-bold uppercase tracking-widest text-muted-foreground">Titelbild-URL (optional)</label>
        <input value={coverUrl} onChange={e => setCoverUrl(e.target.value)}
          className="w-full border border-border bg-input px-3 py-2 text-sm" />
      </div>

      <div>
        <label className="mb-1 block text-xs font-bold uppercase tracking-widest text-muted-foreground">Schlagworte (kommagetrennt)</label>
        <input value={tags} onChange={e => setTags(e.target.value)} placeholder="Einsteiger, Guard, Wettkampf"
          className="w-full border border-border bg-input px-3 py-2 text-sm" />
      </div>
